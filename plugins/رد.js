...
await conn.sendMessage(m.chat, { react: { text: '🥺', key: m.key } })
 ...
     conn.reply(m.chat, `*عمي هو هارلي لايت طبعا 🫶🏻💗*`, m) //wm, null, [['Menu', '#menu']], m) botones :V
    ...
    if (/^بوت$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat, `*اسمي بويكا يا هطف 🗿*`, m) //wm, null, [['Menu', '#menu']], m) botones :V
await conn.sendMessage(m.chat, { react: { text: '🗿', key: m.key } })
}
...
await conn.sendMessage(m.chat, { react: { text: '🗿', key: m.key } })
   ...
   if (/^بويكا$/i.test(m.text) ) { //sem prefixo 
     conn.reply(m.chat,'*قلب بويكا 🐦💗*',m) //wm, null, [['Menu', '#menu']], m) botones :v
await conn.sendMessage(m.chat, { react: { text: '💗', key: m.key } })
   }
 ...
     conn.reply(m.chat, `*الشارع الي وراه ده انا هعملك محضر ابن فاجرة 🐧*`, m) //wm, null, [['Menu', '#menu']], m) botones :V 
