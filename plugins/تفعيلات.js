...
*الحاله:* المطور بس الي يستخدم الامر ده.`.trim();
  ...
  conn.sendMessage(m.chat, {text: `🗂️ الــخــيــار: ${type}\n🎚️ الــحــالـة: ${isEnable ? 'يعمل' : 'لا يعمل'}\n📣 الــي: ${isAll ? 'هذا البوت' : isUser ? '' : 'هذه الدردشه'}`}, {quoted: m});
};
handler.help = ['en', 'dis'].map((v) => v + 'able <option>');
handler.tags = ['group', 'owner'];
handler.command = /^((en|dis)able|(فتح|قفل)ا|(turn)?[01])$/i;
